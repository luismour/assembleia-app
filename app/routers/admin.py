...
def get_telao(db: Session = Depends(get_db)):
    ...
    return {
        "evento": nome,
        "pauta": {
            "titulo": pauta.titulo, "status": pauta.status, "tipo": pauta.tipo,
            "max_escolhas": pauta.max_escolhas, "total_votos": len(votos),
            "resultados": cont, "resultado_final": res
        }
    }
...
def add_grupo_massa(d: GrupoNomesInput, db: Session = Depends(get_db), u: str = Depends(verificar_admin)):
    ...
    if todos:
        try:
            prox = max([int(x.id.split('-')[1]) for x in todos]) + 1
...
def admin_data(db: Session = Depends(get_db), u: str = Depends(verificar_admin)):
    ...
    for p in pautas:
        ...
        if p.status == "ENCERRADA":
            if p.tipo == "SIMPLES":
                final = "APROVADA" if cont["favor"] > cont["contra"] else "REPROVADA"
            else: final = "CONCLUÍDA"

        res.append({
            "id": p.id, "titulo": p.titulo, "status": p.status, "tipo": p.tipo,
            "candidatos": cands, "max_escolhas": p.max_escolhas,
            "total_votos": len(votos), "esperados": total_users,
            "resultados": cont, "resultado_final": final, "votos_detalhados": dets
        })
...
def add_pauta(d: PautaInput, db: Session = Depends(get_db), u: str = Depends(verificar_admin)):
    ...
    nova = models.Pauta(
        id=str(uuid.uuid4()), titulo=d.titulo, assembleia_id=asm.id,
        tipo=d.tipo, max_escolhas=d.max_escolhas,
        candidatos_str=json.dumps(d.candidatos)
    )
    db.add(nova)
    db.commit()
    return nova
